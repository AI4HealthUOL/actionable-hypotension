...
CREATE TABLE ce_approach.split_all_subjects AS
SELECT 
  subject_id,
  CASE
    WHEN ( ('x'||substr(md5(subject_id::text),1,8))::bit(32)::bigint % 100 ) < 70 THEN 'train'
    WHEN ( ('x'||substr(md5(subject_id::text),1,8))::bit(32)::bigint % 100 ) < 85 THEN 'val'
    ELSE 'test'
  END AS split
FROM (
  SELECT DISTINCT subject_id FROM (
    SELECT subject_id FROM mix_windows
    UNION
    SELECT subject_id FROM invasive_windows
    UNION
    SELECT subject_id FROM noninvasive_windows
  ) AS all_ids
) AS unique_subjects;